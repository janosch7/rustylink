//! Behaviour tests for per-block renderer helpers (captions, label strings,
//! switch criteria, multiport selection).

use rustylink::model::{Block, PortCounts};
use rustylink::simulink_libraries::metadata::BlockMetadata;
...
use rustylink::simulink_libraries::stubs::create_stub_block;
...
fn multiport_block(ins: u32) -> Block {
    let mut block = create_stub_block("MultiPortSwitch", ins, 1);
